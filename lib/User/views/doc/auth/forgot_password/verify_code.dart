...
class DocVerifyCode extends StatelessWidget {
  const DocVerifyCode({Key? key}) : super(key: key);
      ...
      body: Container(
        ...
        child: ListView(children: [
          ...
          GetBuilder<ForgotPasswordControllerImp>(builder: (ctrl){
            return CustomButtonAuth(text: "Check", onPressed: () {
              ctrl.checkEmail(ctrl.email.text).then((checkMail){
                if(checkMail){
                  debugPrint('Status: Success go check your email address!');
                  // ctrl.goToVerifyCode();
                }else{
